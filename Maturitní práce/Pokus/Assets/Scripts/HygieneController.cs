...
public class HygieneController : MonoBehaviour {
	...
	void Update () {
		...
		if (isPressed == true && (hitInfo.collider != null || hitInfoRightPickUp.collider != null || hitInfoLeftPickUp.collider != null)) {
			...
			// For cyklus pro snížení hodnoty v proměnné 'hygiene' ve scriptu PlayerController
			for (int i = 0; i < 20; i++) {
				if (PlayerController.hygiene < PlayerController.maxHygiene && PlayerController.hygiene > 0)
					PlayerController.hygiene--;
			}
		}
